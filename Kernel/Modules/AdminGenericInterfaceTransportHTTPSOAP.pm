...
# --
# $Id: AdminGenericInterfaceTransportHTTPSOAP.pm,v 1.1 2011/05/18 03:33:07 cg Exp $
# --
...
sub Run {
    ...
    # ------------------------------------------------------------ #
        ...
        $WebserviceData->{Config}->{Endpoint}   = $GetParam->{Endpoint};
        $WebserviceData->{Config}->{NameSpace}  = $GetParam->{NameSpace};
        $WebserviceData->{Config}->{Encoding}   = $GetParam->{Encoding};
        $WebserviceData->{Config}->{SOAPAction} = $GetParam->{SOAPAction};
        $WebserviceData->{Config}->{MaxLength}  = $GetParam->{MaxLength};
    ...
    #        }
        ...
        return $Self->_ShowOverview( %Param, Action => 'Overview', );
    }
    ...
    # ------------------------------------------------------------ #
        ...
        return $Self->_ShowEdit( Action => 'Add', );
    }
    ...
    # ------------------------------------------------------------ #
        ...
        $WebserviceData->{Config}->{RemoteSystem} =
            $GetParam->{RemoteSystem};
        $WebserviceData->{Config}->{Protocol} = $GetParam->{Protocol};
        $WebserviceData->{Config}->{Debugger}->{DebugThreshold} =
            $GetParam->{DebugThreshold};
        $WebserviceData->{Config}->{Debugger}->{TestMode} =
            $GetParam->{TestMode};
        $WebserviceData->{ValidID} = $GetParam->{ValidID};

        for my $CommunicationType (qw( Provider Requester )) {

            $WebserviceData->{Config}->{$CommunicationType}->{Transport}
                ->{Type} = $GetParam->{ $CommunicationType . 'Transport' };
        }
            ...
            )
        {
            ...
            # add server error error class
            $Error{NameServerError} = 'ServerError';
            $Error{NameServerErrorMessage} =
                'There is another webservice with the same name.';
        }
...
sub _ShowOverview {
    ...
    my $WebserviceList =
        $Self->{WebserviceObject}->WebserviceList( Valid => 0 );
    ...
    #otherwise show all webservices
        ...
        for my $WebserviceID ( keys %{$WebserviceList} ) {
            ...
            my $Webservice =
                $Self->{WebserviceObject}
                ->WebserviceGet( ID => $WebserviceID );
            next WEBSERVICEID if !$Webservice;

            # convert ValidID to text
            my $ValidStrg =
                $Self->{ValidObject}
                ->ValidLookup( ValidID => $Webservice->{ValidID}, );
...
sub _ShowEdit {
    ...
    if ( $Param{Action} eq 'Change' && $WebserviceData->{Name} ) {
        $Self->{LayoutObject}->Block(
            ...
            Data => {
                Name => $WebserviceData->{Name},
                Link => 'Action=AdminGenericInterfaceWebservice;Subaction='
                    . $Param{Action}
                    . ';WebserviceID='
                    . $Param{WebserviceID},
                Nav => '',
    ...
    for my $Transport ( keys %{ $Self->{GITransportConfig} } ) {
        next if !$Transport;
        $GITransports{$Transport} =
            $Self->{GITransportConfig}->{$Transport}->{ConfigDialog};
    }
    ...
    for my $Operation ( keys %{ $Self->{GIOperationConfig} } ) {
        next if !$Operation;
        $GIOperations{$Operation} =
            $Self->{GIOperationConfig}->{$Operation}->{ConfigDialog};
    }
    ...
    for my $Invoker ( keys %{ $Self->{GIInvokerConfig} } ) {
        next if !$Invoker;
        $GIInvokers{$Invoker} =
            $Self->{GIInvokerConfig}->{$Invoker}->{ConfigDialog};
    }
    ...
    for my $CommunicationType (qw(Provider Requester)) {
        ...
        my %GIControllers =
            %{ $CommTypeConfig{$CommunicationType}->{ControllerData} };
        ...
        my $ControllersStrg = $Self->{LayoutObject}->BuildSelection(
            Data => \@ControllerList,
            Name => $CommTypeConfig{$CommunicationType}->{ActionType}
                . 'List',
            Sort => 'AlphanumericValue',
        );
        ...
        $Self->{LayoutObject}->Block(
            ...
            Data => {
                ...
                ActionType =>
                    $CommTypeConfig{$CommunicationType}->{ActionType},
                ControllersStrg => $ControllersStrg,
                ActionsTitle =>
                    $CommTypeConfig{$CommunicationType}->{ActionsTitle},
                }
        );

        if (
            !IsHashRefWithData(
                $CommTypeConfig{$CommunicationType}->{ActionsConfig}
            )
            )
        {
            $Self->{LayoutObject}->Block(
                ...
                )
            {
                ...
                my $Mapping = 'No';
                if (
                    $ActionDails->{MappingInbound}
                    or $ActionDails->{MappingInbound}
                    )
                {
                    $Mapping = 'Yes';
                }
