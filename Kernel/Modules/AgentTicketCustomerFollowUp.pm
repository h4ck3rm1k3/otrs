...
# --
# $Id: AgentTicketCustomerFollowUp.pm,v 1.12 2007/10/02 10:32:22 mh Exp $
# --
    ...
    if ( $Self->{Subaction} eq '' || !$Self->{Subaction} ) {
        ...
        if (my $ArticleID = $Self->{TicketObject}->ArticleCreate(
                ...
                ArticleType => $Self->{ConfigObject}->Get('Ticket::Frontend::CustomerTicketMessage')
                    ->{ArticleType},
                SenderType => $Self->{ConfigObject}->Get('Ticket::Frontend::CustomerTicketMessage')
                    ->{SenderType},
                From        => $From,
                Subject     => $Subject,
                Body        => $Text,
                ...
                HistoryType => $Self->{ConfigObject}->Get('Ticket::Frontend::CustomerTicketMessage')
                    ->{HistoryType},
                HistoryComment =>
                    $Self->{ConfigObject}->Get('Ticket::Frontend::CustomerTicketMessage')
                    ->{HistoryComment} || '%%',
            )
            )
        {
