...
# --
# $Id: AdminCustomerUser.pm,v 1.43.2.1 2010/03/12 09:44:39 martin Exp $
# --
...
sub Run {
    ...
    if ($AddedUID) {
        ...
        my $URL           = '';
        my $UserHTMLQuote = $Self->{LayoutObject}->LinkEncode($AddedUID);
        my $UserQuote     = $Self->{LayoutObject}->Ascii2Html( Text => $AddedUID );
    ...
    if ($Search) {
        ...
        if (%UserList) {
            foreach ( sort keys %UserList ) {
                ...
                if ( $Nav eq 'None' ) {
                    $AddLink
                        = "<a href=\"\" onclick=\"updateMessage('$_')\">\$Text{\"Take this Customer\"}</a>";
                }
                else {
                    $AddLink = $_;
                }
    ...
    # update action
            ...
            )
        {
            ...
            my %UserData
                = $Self->{CustomerUserObject}->CustomerUserDataGet( User => $GetParam{ID} );
