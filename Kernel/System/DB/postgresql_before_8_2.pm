...
sub LoadPreferences {
    ...
    # There is a known error that this driver causes warnings when strings are escaped,
    #   and error messages on the initial-insert file of OTRS.
    # Both are fixed in the newer postgresql driver which requires at least PostgreSQL 8.2 and
    #   will not be fixed in this driver for technical reasons and because you can still use
    #   OTRS with this driver without a problem.
