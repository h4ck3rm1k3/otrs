...
# --
# $Id: AgentInterface.pm,v 1.9 2007/09/29 10:54:31 mh Exp $
# --
...
sub Run {
    ...
    if ( $Param{GetParam}->{'X-OTRS-Loop'} ) {
        ...
        if ( $Self->{Debug} > 1 ) {
            $Self->{LogObject}->Log(
                Priority => 'debug',
                Message =>
                    "Ignore Message for AgentEmailinterface because of loop header (MessageID:$Param{GetParam}->{'Message-ID'})!",
            );
        }
    ...
    if ( !$Param{TicketID} ) {
        if ( $Self->{Debug} > 1 ) {
            $Self->{LogObject}->Log(
                Priority => 'debug',
                Message =>
                    "No email for AgentInterface because of missing TicketID (MessageID:$Param{GetParam}->{'Message-ID'})!",
            );
        }
        return 1;
    }
    ...
    if ( !%Command ) {
        $Self->{LogObject}->Log(
            Priority => 'notice',
            Message =>
                "No commands in AgentInterface email found, send reject to agent (MessageID:$Param{GetParam}->{'Message-ID'})!",
        );
            ...
            if ( $Config{AgentInterfaceAddress} =~ /^\Q$Address\E$/i ) {
                ...
                if ( $Self->{Debug} > 1 ) {
                    $Self->{LogObject}->Log(
                        Priority => 'debug',
                        Message =>
                            "Email for AgentInterface: $_ (MessageID:$Param{GetParam}->{'Message-ID'})!",
                    );
                }
            }
        }
        if ( !$ForAgentInterface ) {
            if ( $Self->{Debug} > 1 ) {
                $Self->{LogObject}->Log(
                    Priority => 'debug',
                    Message =>
                        "Email not for AgentInterface: $_ (MessageID:$Param{GetParam}->{'Message-ID'})!",
                );
            }
            return 1;
        }
    ...
    if ( !$UserID ) {
        $Self->{LogObject}->Log(
            Priority => 'notice',
            Message =>
                "No valid sender for AgentInterface: $From (MessageID:$Param{GetParam}->{'Message-ID'})!",
        );
    ...
    if ( $Command{get} ) {
        ...
        $Self->{LogObject}->Log(
            Priority => 'notice',
            Message =>
                "Sent requested ticket ($Ticket{TicketNumber}) to $Param{GetParam}->{From}: $From (MessageID:$Param{GetParam}->{'Message-ID'})!",
        );
    ...
    if ( $Command{lock} ) {
        ...
        if ( !$Self->{TicketObject}->LockIsTicketLocked( TicketID => $Param{TicketID} ) ) {
            ...
            my ( $OwnerID, $OwnerLogin )
                = $Self->{TicketObject}->OwnerCheck( TicketID => $Param{TicketID}, );
            if ( $OwnerID != $UserID ) {
                ...
                $Self->{LogObject}->Log(
                    Priority => 'notice',
                    Message =>
                        "Reject AgentInterface message because of other owner lock: $From (MessageID:$Param{GetParam}->{'Message-ID'})!",
                );
                ...
                $Self->{EmailObject}->Send(
                    ...
                    Charset => 'us-ascii',
                    Body =>
                        "Sorry, AgentEmail rejected because Ticket is already locked for somebody else!",
                    Loop => 1,
                );
    ...
    if ( $Command{send} ) {
        ...
        if ( $Customer{UserEmail} && $Data{ToEmail} !~ /^\Q$Customer{UserEmail}\E$/i ) {
            $Self->{LogObject}->Log(
                Priority => 'notice',
                Message =>
                    "Added customer database email to Cc: $Customer{UserEmail} (MessageID:$Param{GetParam}->{'Message-ID'})!",
            );
            if ( $Data{Cc} ) {
        ...
        $Self->{LogObject}->Log(
            Priority => 'notice',
            Message =>
                "Send AgentInterface message to $NewRecipient (MessageID:$Param{GetParam}->{'Message-ID'})!",
        );
    }
