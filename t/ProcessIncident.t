# --
# ProcessIncident.t - ProcessIncident Invoker tests
# Copyright (C) 2001-2011 OTRS AG, http://otrs.org/
# --
# $Id: ProcessIncident.t,v 1.1 2011/05/02 14:24:35 sb Exp $
# --
...
my $WebserviceID = $WebserviceObject->WebserviceAdd(
    ...
    Name    => "ProcessIncidentInvokerTest - . $RandomID",
    ValidID => 1,
    UserID  => 1,
);
...
my $InvokerObject = Kernel::GenericInterface::Invoker->new(
    %{$Self},
    DebuggerObject => $DebuggerObject,
    InvokerType    => 'SolMan::ProcessIncident',
);
$Self->IsNot(
    ref $InvokerObject,
...
for my $Test (@Tests) {
    if ( $Test->{PrepareRequest} ) {
        ...
        $Self->Is(
            ref $Result,
            'HASH',
            "Test $Test->{Name}: ProcessIncident PrepareRequest response",
        );
        ...
        $Self->Is(
            ...
            "Test $Test->{Name}: ProcessIncident PrepareRequest",
        );
        ...
        $Self->Is(
            ref $Result->{Data},
            ref $ResponseData,
            "Test $Test->{Name}: ProcessIncident PrepareRequest Data ref",
        );
        ...
        if ( !$Test->{PrepareRequest}->{Success} ) {
            $Self->IsNot(
                $Result->{ErrorMessage},
                undef,
                "Test $Test->{Name}: ProcessIncident PrepareRequest ErrorMessage",
            );
        }
        else {
            ...
            $Self->IsNot(
                ...
                undef,
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctAdditionalInfos",
            );
            ...
            $Self->IsNot(
                ...
                undef,
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctAttachments",
            );

            $Self->Is(
                ref $Result->{Data}->{IctHead},
                'HASH',
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctHead ref",
            );

            $Self->Is(
                $Result->{Data}->{IctHead}->{AgentId},
                1,
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctHead AgentId",
            );

            $Self->Is(
                $Result->{Data}->{IctHead}->{Language},
                $CustomerUser{UserLanguage} || 'en',
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctHead Language",
            );

            $Self->Is(
                $Result->{Data}->{IctHead}->{Priority},
                $Ticket{Priority},
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctHead Priority",
            );

            $Self->Is(
                $Result->{Data}->{IctHead}->{ProviderGuid},
                $WebserviceConfig->{Requester}->{Invoker}->{ProcessIncident}->{RemoteSystemGuid},
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctHead ProviderGuid",
            );

            $Self->Is(
                $Result->{Data}->{IctHead}->{ReporterId},
                $Ticket{CustomerUserID},
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctHead ReporterId",
            );

            $Self->IsNot(
                $Result->{Data}->{IctHead}->{RequestedBegin},
                '',
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctHead RequestedBegin",
            );

            $Self->IsNot(
                $Result->{Data}->{IctHead}->{RequestedEnd},
                '',
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctHead RequestedEnd",
            );

            $Self->True(
                $Result->{Data}->{IctHead}->{RequestedBegin}
                    lt $Result->{Data}->{IctHead}->{RequestedEnd},
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctHead "
            ...
            $Self->Is(
                ...
                $OTRSystemGuid,
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctHead RequesterGuid",
            );

            $Self->Is(
                $Result->{Data}->{IctHead}->{ShortDescription},
                $Ticket{Title},
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctHead ShortDescription",
            );

            $Self->Is(
                $Result->{Data}->{IctId},
                $Ticket{TicketNumber},
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctId",
            );

            $Self->Is(
                ref $Result->{Data}->{IctPersons},
                'HASH',
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctPersons ref",
            );

            $Self->Is(
                ref $Result->{Data}->{IctPersons}->{item},
                'ARRAY',
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctPersons item ref",
            );

            $Self->Is(
                scalar @{ $Result->{Data}->{IctPersons}->{item} },
                2,
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctPersons item count",
            );
            ...
            for my $ItemCounter ( 0 .. 1 ) {
                ...
                $Self->Is(
                    ...
                    'HASH',
                    "Test $Test->{Name}: ProcessIncident PrepareRequest IctPersons "
            ...
            $Self->IsNot(
                ...
                undef,
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctSapNotes",
            );
            ...
            $Self->IsNot(
                ...
                undef,
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctSolutions",
            );

            $Self->Is(
                ref $Result->{Data}->{IctStatements},
                'HASH',
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctStatements ref",
            );
            ...
            $Self->Is(
                ...
                'ARRAY',
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctStatements item ref",
            );
            ...
            $Self->Is(
                ...
                'HASH',
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctStatements item [0] ref",
            );

            $Self->Is(
                $Result->{Data}->{IctStatements}->{item}[0]->{Language},
                $CustomerUser{UserLanguage} || 'en',
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctStatements item [0] "
                    . "Language",
            );
            ...
            $Self->IsNot(
                ...
                '',
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctTimestamp",
            );
            ...
            $Self->Is(
                $Result->{Data}->{IctUrls},
                '',
                "Test $Test->{Name}: ProcessIncident PrepareRequest IctUrls",
            );
        }
    }
    ...
    if ( $Test->{HandleResponse} ) {
        ...
        $Self->Is(
            ref $Result,
            'HASH',
            "Test $Test->{Name}: ProcessIncident HandleResponse response",
        );
        ...
        if ( $Test->{HandleResponse}->{Success} ) {
            ...
            $Self->Is(
                $Result->{Success},
                '1',
                "Test $Test->{Name}: ProcessIncident HandleResponse success",
            );
            ...
            if ( ref $Test->{HandleResponse}->{Data}->{Errors} eq 'HASH' ) {
                ...
                $Self->IsNot(
                    $Result->{ErrorMessage},
                    '',
                    "Test $Test->{Name}: ProcessIncident HandleResponse error message not empty",
                );
                ...
                $Self->Is(
                    ...
                    "Test $Test->{Name}: ProcessIncident PrepareRequest ReSchedule",
                );
                ...
                $Self->Is(
                    $Result->{PersonMaps},
                    undef,
                    "Test $Test->{Name}: ProcessIncident HandleResponse no PersonMaps",
                );
                ...
                $Self->Is(
                    $Result->{PrdIctId},
                    undef,
                    "Test $Test->{Name}: ProcessIncident HandleResponse no PrdIctId",
                );
            ...
            $Self->Is(
                $Result->{Success},
                '0',
                "Test $Test->{Name}: ProcessIncident HandleResponse not success",
            );
            ...
            $Self->IsNot(
                $Result->{ErrorMessage},
                undef,
                "Test $Test->{Name}: ProcessIncident HandleResponse ErrorMessage",
            );
        }
